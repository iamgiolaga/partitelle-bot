...
class TestSetPitch(unittest.TestCase):
    ...
    def test_set_pitch(self, mock_find_row_by_chat_id, mock_update_pitch_on_db, mock_get_sender_name, mock_find_all_info_by_chat_id, mock_format_summary, mock_print_new_summary, mock_update_bot_last_message_id_on_db):
        ...
        with self.subTest("Valid arguments provided and print new summary"):
            ...
            mock_find_row_by_chat_id.return_value = "row"
            mock_find_all_info_by_chat_id.return_value = (["@player1", "@player2"], "Monday", "20:00", "10", "default_message", "Campo Test", None, None)
            ...
            mock_find_row_by_chat_id.assert_called_once_with(mock_update.message.chat_id)
            mock_update_pitch_on_db.assert_called_once_with(mock_update.message.chat_id, "Campo Test")
            mock_context.bot.send_message.assert_called_once_with(
                chat_id=mock_update.effective_chat.id,
                parse_mode='markdown',
                text="Ok, @John! Ho aggiornato il campo!"
            )
            mock_format_summary.assert_called_once_with(["@player1", "@player2"], "Monday", "20:00", "10", "default_message", "Campo Test")
