...
class TestSetNumber(unittest.TestCase):
    ...
    def test_set_number(
        self,
        mock_remove_job_if_exists,
        mock_update_teams_on_db,
        mock_update_target_on_db,
        ...
        with self.subTest("Teams is not None"):
            ...
            mock_find_all_info_by_chat_id.assert_has_calls(
                [call(mock_update.message.chat_id), call(mock_update.message.chat_id)]
            )
            mock_get_sender_name.assert_called_once_with(mock_update)
            mock_update_teams_on_db.assert_called_once_with(
                mock_update.message.chat_id, None
            )
            mock_remove_job_if_exists.assert_called_once_with(
                str(mock_update.message.chat_id), mock_context
            )
            mock_update_target_on_db.assert_called_once_with(
                mock_update.message.chat_id, 8
            )
            ...
            mock_context.bot.send_message.assert_has_calls(
                [
                    ...
                    call(
                        chat_id=mock_update.effective_chat.id,
                        parse_mode="markdown",
                        text="Ok, @John! Ho impostato il numero di partecipanti a 8",
                    ),
                ]
            )
            mock_edit_summary.assert_called_once_with(
                "formatted_summary", 123, mock_update, mock_context
            )
            ...
            mock_context.bot.send_message.assert_has_calls(
                [
                    ...
                    call(
                        chat_id=mock_update.effective_chat.id,
                        parse_mode="markdown",
                        text="Ok, @John! Ho impostato il numero di partecipanti a 2",
                    ),
                ]
            )
