...
def create_bamboo_chat_id_row(chat_id):
    ...
    connection.execute(
        "INSERT INTO all_players_test (chat_id, players, day, time, target, custom_message, pitch, teams, bot_last_message_id) "
...
def delete_row_on_db(chat_id):
    connection.execute('DELETE FROM all_players_test WHERE chat_id=%s', [str(chat_id)])

def is_already_present(chat_id, name):
    connection.execute('SELECT players FROM all_players_test WHERE chat_id=%s', [str(chat_id)])
...
def create_chat_id_row(chat_id):
    default_day = "Mercoledì " + compute_next_wednesday()
    connection.execute("INSERT INTO all_players_test (chat_id, players, day, time, target, custom_message, pitch, teams, bot_last_message_id) "
...
def find_all_info_by_chat_id(chat_id):
    connection.execute('SELECT players, day, time, target, custom_message, pitch, teams, bot_last_message_id FROM all_players_test WHERE chat_id=%s', [str(chat_id)])
    return connection.fetchone()

def find_row_by_chat_id(chat_id):
    connection.execute('SELECT chat_id, players FROM all_players_test WHERE chat_id=%s', [str(chat_id)])
    return connection.fetchone()

def update_bot_last_message_id_on_db(chat_id, msg_id):
    connection.execute('UPDATE all_players_test SET bot_last_message_id = %s WHERE chat_id= %s', [str(msg_id), str(chat_id)])

def update_target_on_db(chat_id, target):
    connection.execute('UPDATE all_players_test SET target = %s WHERE chat_id= %s', [str(target), str(chat_id)])

def update_day_on_db(chat_id, day):
    connection.execute("UPDATE all_players_test SET day = %s WHERE chat_id= %s", [day, str(chat_id)])

def update_time_on_db(chat_id, time):
    connection.execute("UPDATE all_players_test SET time = %s WHERE chat_id= %s", [time, str(chat_id)])

def update_description_on_db(chat_id, description):
    connection.execute("UPDATE all_players_test SET custom_message = %s WHERE chat_id= %s", [description, str(chat_id)])

def update_pitch_on_db(chat_id, pitch):
    connection.execute("UPDATE all_players_test SET pitch = %s WHERE chat_id= %s", [pitch, str(chat_id)])

def update_teams_on_db(chat_id, teams):
    connection.execute("UPDATE all_players_test SET teams = %s WHERE chat_id= %s", [teams, str(chat_id)])

def update_players_on_db(chat_id, new_entry, action):
    connection.execute('SELECT players FROM all_players_test WHERE chat_id=%s', [str(chat_id)])
    current_players = connection.fetchone()
    if current_players[0] is None:
        connection.execute("UPDATE all_players_test SET players = %s WHERE chat_id=%s", ['{'+new_entry+'}', str(chat_id)])
    else:
        result = "{"
        if action == "add":
        ...
        connection.execute('UPDATE all_players_test SET players = %s WHERE chat_id=%s', [result, str(chat_id)])
